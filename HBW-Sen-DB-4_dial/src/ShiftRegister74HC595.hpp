...
void ShiftRegister74HC595<Size>::setAll_P(const uint8_t * digitalValuesProgmem)
{
    ...
    // memcpy_P( _digitalValues, digitalValuesProgmem, Size); // TODO rm - testing
    updateRegisters();
}
#endif
...
{
// Serial.print(F("shift:")); // TODO rm - testing
    for (int i = Size - 1; i >= 0; i--) {
        shiftOut(_serialDataPin, _clockPin, MSBFIRST, _digitalValues[i]);
    //     uint8_t val = _digitalValues[i];

	// for (uint8_t b = 0; b < 8; b++)  {
	// 	digitalWrite(_serialDataPin, !!(val & (1 << (7 - b))));
    //     // delayMicroseconds(800);
	// 	digitalWrite(_clockPin, HIGH);
    //     // delayMicroseconds(200);
	// 	digitalWrite(_clockPin, LOW);		
	// }
// Serial.print(_digitalValues[i]); // TODO rm - testing
    }
    
    digitalWrite(_latchPin, HIGH); 
    digitalWrite(_latchPin, LOW);
// Serial.println(""); // TODO rm - testing
}
