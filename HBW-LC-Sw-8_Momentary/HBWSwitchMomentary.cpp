...
//#include "HBWSwitch.h"
#include "HBWSwitchMomentary.h"


// Switches
//HBWSwitchMomentary::HBWSwitchMomentary(uint8_t _pin, hbw_config_switch_momentary* _config) {
//    pin = _pin;
//    config = _config;
//    clearFeedback();
...
// void HBWSwitchMomentary::afterReadConfig() {
...
void HBWSwitchMomentary::set(HBWDevice* device, uint8_t length, uint8_t const * const data) {
...
// uint8_t HBWSwitchMomentary::get(uint8_t* data) {
	// if (currentState)
		// (*data) = 200;
	// else
		// (*data) = 0;
...
void HBWSwitchMomentary::loop(HBWDevice* device, uint8_t channel) {
