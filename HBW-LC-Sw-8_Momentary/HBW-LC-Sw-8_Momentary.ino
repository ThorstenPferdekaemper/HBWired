//*******************************************************************
//
// HBW-LC-Sw-8 (HBWSwitch Momentary version)
//
...
struct hbw_config {
  ...
  uint8_t              :7;   // 0x06:1-7
  hbw_config_switch_momentary switchMomCfg[NUM_CHANNELS]; // 0x07-0x... ? (address step 2)
} hbwconfig;
...
void setup()
{
  ...
  for(uint8_t i = 0; i < NUM_CHANNELS; i++) {
    switches[i] = new HBWSwitchMomentary(pins[i], &(hbwconfig.switchMomCfg[i]));
  }
